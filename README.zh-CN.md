...
> 克隆 https://github.com/PluteW/isola ，安装依赖，生成配置，并使 `python -m isola doctor` 全部通过。
...
```bash
python -m isola chat --text "支付服务：梳理这次重构的回滚方案"
python -m isola chat --text "数据平台：排查昨晚的同步任务延迟"
