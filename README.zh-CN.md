...
Isola 支持两种集成模式，共用同一套核心。**委托模式**下，Isola 持入口，把判定好归属的消息投递到该项目的后端会话——就是上面描述的形态。**记忆服务模式**下，agent 自己就是入口（Claude Code、Codex）：它通过 MCP 调 Isola 判定归属（`route`）、召回该项目记忆（`recall`），自己执行，再把结论写回（`remember`）。这种模式下 Isola 既不投递、也不碰任何后端，纯粹是项目级的归属与记忆层。归属判定、软归属、纠正和写入门链，两种模式走的是同一份代码。
...
- **两种集成模式，同一核心**：委托模式（Isola 持入口、投递给后端）与记忆服务模式（Claude Code、Codex 这类 agent 通过 MCP 调 Isola、自己执行）。两者复用同一套归属、隔离、纠正与记忆代码。
...
**记忆服务模式（MCP）。** 对自身就是入口的 agent（Claude Code、Codex），把 Isola 当 MCP server 跑，而不是持入口的投递者。agent 自己调 `route` → `recall` →（自己执行）→ `remember`；Isola 不投递。

```bash
pip install -e ".[mcp]"             # 装上官方 mcp SDK 与 isola-mcp 命令
...
当前提供两种可直接落地的形态。CLI 同步档（委托模式）：单机配置、项目级路由、独立 harness 会话、持久化状态、记忆隔离、纠正闭环与机器可读自检。以及 MCP 记忆服务模式（`isola-mcp`，stdio）：把归属、召回、确认、写回、纠正暴露成工具，供自己执行的 agent 调用。两者共用同一核心边界，也为后续服务化形态提供稳定的状态基础。
